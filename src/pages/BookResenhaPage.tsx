  ...
  <div className="min-h-screen bg-background">
      ...
      <div className="max-w-[1120px] mx-auto px-[clamp(16px,4vw,48px)]">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
          ...
          <div className="flex-1">
            <span className="inline-block font-display text-[0.42rem] tracking-[0.14em] uppercase bg-bx-700 text-white px-2.5 py-1 mb-4">
              {book.category}
            </span>
            <h1 className="font-heading text-[clamp(1.6rem,3.5vw,2rem)] italic font-semibold text-gy-900 leading-[1.15] mb-2">
              {book.title}
            </h1>
            <p className="font-display text-[0.5rem] tracking-[0.14em] uppercase text-gy-400 mb-3">
              {book.author}
            </p>
            <StarRating count={book.stars} />
            <div className="w-full h-px bg-gy-200 my-4" />
            <p className="font-display text-[0.46rem] tracking-[0.12em] uppercase text-gy-400">
          ...
          <div className={`w-[180px] shrink-0 aspect-[2/3] ${book.coverBg} flex flex-col items-center justify-center p-6`}>
            <h3 className="font-heading text-[1rem] italic font-semibold text-white text-center leading-[1.3] mb-1">
              {book.title}
            </h3>
            <p className="font-display text-[0.42rem] tracking-[0.14em] uppercase text-white/60">
              {book.author}
            </p>
          </div>
        </div>
    ...
    <article className="max-w-[680px] mx-auto py-12 px-[clamp(16px,4vw,48px)]">
      ...
      <blockquote className="border-l-4 border-gd-600 pl-6 my-8">
        <p className="font-heading italic text-[1.3rem] text-gy-700 leading-[1.5]">
    ...
    <div className="max-w-[680px] mx-auto px-[clamp(16px,4vw,48px)] pb-12">
      <div className="bg-surface-warm p-6 flex gap-5 items-start">
        ...
        <div className="flex-1">
          <h3 className="font-heading text-[1.1rem] italic font-semibold text-gy-900 leading-[1.25] mb-1">
            {book.title}
          </h3>
          <p className="font-display text-[0.46rem] tracking-[0.12em] uppercase text-gy-400 mb-3">
            {book.author}
          </p>
          <a
            href="#"
            className="inline-block font-display text-[0.5rem] tracking-[0.16em] uppercase bg-bx-700 text-white px-4 py-2 hover:bg-bx-600 transition-colors"
          >
            Comprar livro →
          </a>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {moreReviews.map((b) => (
            <Link to="/resenhas/exemplo" key={b.title} className="group">
              ...
              <h4 className="font-heading text-[1.15rem] font-semibold italic text-gy-900 leading-[1.25] mt-1 mb-1">
                {b.title}
              </h4>
              <p className="font-display text-[0.46rem] tracking-[0.12em] uppercase text-gy-400">
                {b.author}
